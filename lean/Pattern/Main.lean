/-
  Termination of the standard sequences with any number of rows, with labels in R_r
  (`r` = number of rows):

    ◁ₖ = <_{k+1}.

  An array with `r` rows is standard if it is reachable from some `(0,…,0)(1,…,1)⋯(n,…,n)`.
  r = 1: primitive sequences, r = 2: pair sequences, r = 3: trio sequences, and so on.
-/
...
theorem stable_stair {r : ℕ} (n : ℕ) :
    ∃ f, Stable (labelSystemGen r) (stair r n) f := by
...
theorem std_stable {r : ℕ} {A : Arr r} (hA : Std r A) (h0 : 0 < A.len) :
    ∃ f, Stable (labelSystemGen r) A f := by
  induction hA with
  ...
  | @step A N hA ih =>
    ...
    · obtain ⟨f, hf⟩ := ih (Nat.pos_of_ne_zero hlen)
      obtain ⟨g, hg, _⟩ := descent (labelSystemGen r) hf (Nat.pos_of_ne_zero hlen) N h0
      exact ⟨g, hg⟩

/-- The labels chosen along an expansion sequence with no empty term. -/
private noncomputable def chain {r : ℕ} {A : Arr r} {n : ℕ → ℕ}
...
private theorem chain_lt {r : ℕ} {A : Arr r} {n : ℕ → ℕ}
    (hpos : ∀ t, 0 < (seq A n t).len) {f₀ : ℕ → Ordinal.{0}}
    (hf₀ : Stable (labelSystemGen r) A f₀) (t : ℕ) :
    ht (labelSystemGen r) (seq A n (t + 1)) (chain hpos hf₀ (t + 1)).1
      < ht (labelSystemGen r) (seq A n t) (chain hpos hf₀ t).1 :=
  (descent (labelSystemGen r) (chain hpos hf₀ t).2 (hpos t) (n t)
    (hpos (t + 1))).choose_spec.2

/-- **Termination**: for every `r`, every expansion sequence starting from a standard array with
`r` rows reaches the empty array. -/
theorem terminates {r : ℕ} {A : Arr r} (hA : Std r A) (n : ℕ → ℕ) :
    ∃ T, (seq A n T).len = 0 := by
  by_contra hcon
  push Not at hcon
  have hpos : ∀ t, 0 < (seq A n t).len := fun t => Nat.pos_of_ne_zero (hcon t)
  obtain ⟨f₀, hf₀⟩ := std_stable hA (hpos 0)
  have hdesc := chain_lt hpos hf₀
  obtain ⟨β, ⟨t₀, rfl⟩, hmin⟩ :=
    (wellFounded_lt (α := Ordinal.{0})).has_min
      (Set.range fun t => ht (labelSystemGen r) (seq A n t) (chain hpos hf₀ t).1)
...
/-- **Well-foundedness** of one-step expansion on standard arrays with `r` rows. -/
theorem StdR_wf (r : ℕ) : WellFounded (StdR r) := by
  ...
  obtain ⟨T, hT⟩ := terminates (B 0).2 n
